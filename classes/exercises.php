			...
			case "sum":
				if(!$with_dates){
					if($this->results_json!=null){
						...
						$query = "SELECT sum(result) AS `sum` FROM log_entry_regular LEFT JOIN sets ON log_entry_id=log_entry_regular.id left join log_entry ON log_entry_regular.id=log_entry.rel_id WHERE exercise_id=?  AND log_entry.id IS NOT NULL GROUP BY log_entry_id ORDER BY UNIX_TIMESTAMP(begin_time) ASC";
